...
SCRIPT_NAME="00-web-development"
...
function install_composer() {
    log_step "Installing PHP Composer for user scott"
    ...
    # Create directory for Composer in scott's home
    log_info "Creating Composer directory for user scott"
    local scott_home="/home/scott"
    local composer_dir="${scott_home}/.local/bin"
    
    if [[ ! -d "${composer_dir}" ]]; then
        mkdir -p "${composer_dir}"
    fi
    
    # Install Composer for user scott
    log_info "Installing Composer for user scott"
    if ! sudo -u scott php "${composer_setup}" --quiet --install-dir="${composer_dir}" --filename=composer; then
        log_error "Failed to install Composer for user scott"
        rm "${composer_setup}"
        return 1
    fi
    ...
    chown -R scott:scott "${composer_dir}"
    
    # Add Composer to scott's PATH if not already there
    log_info "Adding Composer to scott's PATH"
    local profile_file="${scott_home}/.profile"
    
    if [[ -f "${profile_file}" ]]; then
        if ! grep -q "${composer_dir}" "${profile_file}"; then
            # Add to PATH
            sudo -u scott bash -c "echo 'export PATH=\"\$HOME/.local/bin:\$PATH\"' >> \"${profile_file}\""
    ...
    log_info "Creating system-wide symlink to scott's composer"
    ...
    composer_version=$(sudo -u scott "${composer_dir}/composer" --version) || true
    log_info "Composer ${composer_version} installed successfully for user scott"
    
    set_state "${SCRIPT_NAME}_composer_installed"
    log_success "Composer installed successfully for user scott"
    return 0
}
...
function install_php_tools() {
    ...
    if [[ -n "${SUDO_USER}" ]]; then
        if [[ ! -d "${user_home}/.config/composer" ]]; then
            mkdir -p "${user_home}/.config/composer"
            chown -R "${SUDO_USER}:${SUDO_USER}" "${user_home}/.config/composer"
        fi
    else
    ...
    for package in "${composer_packages[@]}"; do
        ...
        if [[ -n "${SUDO_USER}" ]]; then
            ...
            if ! sudo -u "${SUDO_USER}" composer global require --no-interaction "${package}"; then
    ...
    if [[ -f "${profile_file}" ]]; then
        if ! grep -q "composer/vendor/bin" "${profile_file}"; then
            if [[ -n "${SUDO_USER}" ]]; then
                ...
                sudo -u "${SUDO_USER}" bash -c "echo 'export PATH=\"\$HOME/.config/composer/vendor/bin:\$PATH\"' >> \"${profile_file}\""
            else
