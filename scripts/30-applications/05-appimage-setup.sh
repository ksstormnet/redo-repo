...
# Install Zen Browser AppImage
function install_zen_browser() {
    log_step "Installing Zen Browser AppImage"
    
    if check_state "${SCRIPT_NAME}_zen_browser_installed"; then
        log_info "Zen Browser AppImage is already installed. Skipping..."
        return 0
    fi
    
    # Zen Browser uses an installer script rather than direct download
    local zen_installer_url="https://updates.zen-browser.app/appimage.sh"
    local zen_installer_path="/tmp/zen-installer.sh"
    local user_home
    ...
    log_info "Downloading Zen Browser installer from ${zen_installer_url}"
    
    # Download the installer script
    if ! wget "${zen_installer_url}" -O "${zen_installer_path}"; then
        log_error "Failed to download Zen Browser installer"
        return 1
    fi
    
    # Make it executable
    chmod +x "${zen_installer_path}"
    
    # Run the installer as the actual user
    log_info "Running Zen Browser installer"
    if [[ -n "${SUDO_USER}" ]]; then
        if ! sudo -u "${SUDO_USER}" "${zen_installer_path}"; then
            log_error "Failed to install Zen Browser"
            return 1
        fi
    else
        if ! "${zen_installer_path}"; then
            log_error "Failed to install Zen Browser"
            return 1
        fi
    fi
    
    set_state "${SCRIPT_NAME}_zen_browser_installed"
    log_success "Zen Browser AppImage installed successfully"
    return 0
}
...
function install_rclone_browser() {
    log_step "Installing RcloneBrowser AppImage"
    
    if check_state "${SCRIPT_NAME}_rclone_browser_installed"; then
        log_info "RcloneBrowser AppImage is already installed. Skipping..."
        return 0
    fi
    
    local rclone_browser_url="https://github.com/kapitainsky/RcloneBrowser/releases/download/1.8.0/RcloneBrowser-1.8.0-x86_64.AppImage"
    local rclone_browser_desktop="[Desktop Entry]
Name=Rclone Browser
Comment=GUI for rclone, rsync for cloud storage
Exec=/home/${SUDO_USER}/Applications/RcloneBrowser.AppImage
Icon=rclone-browser
Terminal=false
Type=Application
Categories=Utility;FileTransfer;
Keywords=rclone;cloud;storage;file;sync;
StartupWMClass=rclone-browser"
    
    # Download and install RcloneBrowser
    if download_appimage "RcloneBrowser" "${rclone_browser_url}" "${rclone_browser_desktop}" "true"; then
        log_success "RcloneBrowser AppImage installed successfully"
        return 0
    else
        log_error "Failed to install RcloneBrowser AppImage"
        return 1
    fi
}

# Install mdView AppImage
function install_md_view() {
    log_step "Installing mdView AppImage"
    
    if check_state "${SCRIPT_NAME}_md_view_installed"; then
        log_info "mdView AppImage is already installed. Skipping..."
        return 0
    fi
    
    local md_view_url="https://github.com/qjebbs/vscode-markdown-extended/releases/download/v1.0.1/mdview-1.0.1.AppImage"
    local md_view_desktop="[Desktop Entry]
Name=mdView
Comment=Markdown Viewer
Exec=/home/${SUDO_USER}/Applications/mdView.AppImage %f
Icon=mdview
Terminal=false
Type=Application
Categories=Utility;TextEditor;
MimeType=text/markdown;
Keywords=markdown;viewer;md;
StartupWMClass=mdview"
    
    # Download and install mdView
    if download_appimage "mdView" "${md_view_url}" "${md_view_desktop}" "true"; then
        log_success "mdView AppImage installed successfully"
        return 0
    else
        log_error "Failed to install mdView AppImage"
        return 1
    fi
}

# Install ParseHub AppImage
function install_parse_hub() {
    log_step "Installing ParseHub AppImage"
    
    if check_state "${SCRIPT_NAME}_parse_hub_installed"; then
        log_info "ParseHub AppImage is already installed. Skipping..."
        return 0
    fi
    
    local parse_hub_url="https://www.parsehub.com/static/client/parsehub-linux.AppImage"
    local parse_hub_desktop="[Desktop Entry]
Name=ParseHub
Comment=Web scraping tool
Exec=/home/${SUDO_USER}/Applications/ParseHub.AppImage
Icon=parsehub
Terminal=false
Type=Application
Categories=Development;WebDevelopment;
Keywords=scraping;data;extraction;web;
StartupWMClass=parsehub"
    
    # Download and install ParseHub
    if download_appimage "ParseHub" "${parse_hub_url}" "${parse_hub_desktop}" "true"; then
        log_success "ParseHub AppImage installed successfully"
        return 0
    else
        log_error "Failed to install ParseHub AppImage"
        return 1
    fi
}
...
function setup_appimages() {
    ...
    install_zen_browser || log_warning "Failed to install Zen Browser AppImage"
    ...
    install_rclone_browser || log_warning "Failed to install RcloneBrowser AppImage"
    install_md_view || log_warning "Failed to install mdView AppImage"
    install_parse_hub || log_warning "Failed to install ParseHub AppImage"
