...
function install_image_editing_tools() {
    ...
    # Verify GIMP installation (as a critical component)
    if ! command -v gimp &> /dev/null; then
        log_error "GIMP installation failed"
        # But continue anyway as other applications might have installed
    else
        # Declare first, then assign to avoid masking return values
        local gimp_version
        gimp_version=$(gimp --version | head -n1) || true
        log_info "GIMP ${gimp_version} installed successfully"
    fi
